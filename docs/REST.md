...
Commands:
    ...
    PATCH   // to apply modifications to a resource

Query String Parameters:

    query   | allows to search for field names                  | e.g. Category="incident" and (Title="Desktop screen out of order" or Title="Network logon failure")
    sort    | Retuns collection members in sorted order         | e.g. sort=Urgency or sort=severity or sort=Urgency:ascending,field2:descending
    start   | index of the beginning collection member
    count   | number of members to be incl. in the response     |
    view    | Represents a collection
